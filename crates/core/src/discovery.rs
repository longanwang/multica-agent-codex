use crate::types::{AcpSupport, AgentLaunch, AgentProfile, AgentSource};
...
fn resolve_command(command: &str) -> Option<PathBuf> {
    ...
    for base in common_windows_roots() {
        for candidate in [
            base.join(command),
            base.join(format!("{command}.exe")),
            base.join("bin").join(command),
            base.join("bin").join(format!("{command}.exe")),
        ] {
            if candidate.exists() {
...
pub fn fake_agent_profile() -> AgentProfile {
    AgentProfile {
        ...
        name: "Fixture Fast Reviewer".to_string(),
